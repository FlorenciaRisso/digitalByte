...
const cartService = {
  ...
  calcularTotalCarritoIncremento: function (
    totalCarrito,
    precioProdu,
    descProdu,
    nuevaCantidad
    ...
    let precioConDescuento =
      parseFloat(precioProdu) - parseFloat(precioProdu) * (descProdu / 100);
    let totalDetalle = 0;
  ...
  calcularTotalCarritoDecremento: function (
    totalCarrito,
    precioProdu,
    descProdu,
    nuevaCantidad
  ...
  addProductoAlCarrito: async function (idCarrito, idProducto, cantidad) {
    try {
      ...
      const existeProducto = await this.existeProductoEnCarrito(
        idCarrito,
        idProducto
      );
      ...
      productoDetalle = await db.DetalleCarrito.findOne({
        where: { ID_Carrito: idCarrito, ID_Producto: idProducto },
      });
      if (b == 1) {
      ...
      nuevoTotalCarrito = this.calcularTotalCarritoIncremento(
        existeCarrito[0].Total,
        producto.Precio,
        producto.Descuento,
        cantidadASumar
      );
      return await db.Carritos.update(
        { Total: nuevoTotalCarrito },
        { where: { id: idCarrito } }
      );
    } catch (error) {
  ...
  updateCantidad: async function (idDetalleProducto, nuevaCantidad) {
    try {
      let detalleProductoAnterior = await db.DetalleCarrito.findByPk(
        idDetalleProducto
      );
      let producto = await db.Productos.findByPk(
        detalleProductoAnterior.ID_Producto
      );
      ...
      let detalleProductoActualizado = await db.DetalleCarrito.findByPk(
        idDetalleProducto
      );
      let diferenciaCantidad = nuevaCantidad - detalleProductoAnterior.Cantidad;
      let carrito = await db.Carritos.findByPk(
        detalleProductoActualizado.ID_Carrito
      );
      let nuevoTotalCarrito = carrito.Total;

      if (diferenciaCantidad < 0) {
        // Si estás decrementando la cantidad
        nuevoTotalCarrito = this.calcularTotalCarritoDecremento(
          carrito.Total,
          producto.Precio,
          producto.Descuento,
        ...
        // Si estás incrementando la cantidad
        nuevoTotalCarrito = this.calcularTotalCarritoIncremento(
          carrito.Total,
          producto.Precio,
          producto.Descuento,
  ...
  delete: async function (id) {
    try {
      ...
      let productoEliminar = await db.Productos.findByPk(
        detalleEliminar.ID_Producto
      );
      if (!productoEliminar) {
      ...
      if (!carrito) {
        throw new Error(
          "No se encontró el carrito asociado al detalle del producto eliminado"
        );
      }

      let nuevoTotalCarrito = this.calcularTotalCarritoDecremento(
        carrito.Total,
        productoEliminar.Precio,
        productoEliminar.Descuento,
        detalleEliminar.Cantidad
      );
  ...
  finalizarCompra: async function (carritoId) {
    ...
    try {
      ...
      await Promise.all(
        productosCarrito.map(async (detalle) => {
      ...
      // Cambiar el estado del carrito a 1
      const carrito = await db.Carritos.findByPk(carritoId, { transaction: t });
      carrito.Estado = 1;
      await carrito.save({ transaction: t });

      // Si todas las operaciones se completaron correctamente, confirmamos la transacción
      await t.commit();
  ...
  getCarritoyDetalle: async function (usuarioID) {
    try {
      let carritoDetalle = await db.Carritos.findAll({
        include: [{ association: "Usuarios" }],
        where: { ID_Usuario: usuarioID, estado: 1 },
        raw: true,
      });
      for (let carrito of carritoDetalle) {
        const productos = await this.getProductosCarrito(carrito.id);
